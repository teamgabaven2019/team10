...
module.exports = MessageDispatcher = (function() {
  ...
  function MessageDispatcher(url, id) {
    this.handlePostMessage = __bind(this.handlePostMessage, this);
    if (!id) {
      id = "anonymous";
    }
    this._url = url;
  ...
  MessageDispatcher.prototype.handlePostMessage = function(event) {
    var WebInspector, data;
    try {
    ...
    if (data.command === "weinre") {
      if (data.deviceId && data.deviceName) {
        this._interfaces['WebInspector'].panels.remote.setTargetName(data.deviceId, data.deviceName);
      }
      this._handleMessage({
    ...
    if (data.command = "resize") {
      WebInspector = this._interfaces["WebInspector"];
      return WebInspector.windowResize();
    }
  };
